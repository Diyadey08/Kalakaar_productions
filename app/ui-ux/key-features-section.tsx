    ...
    <section className="w-full bg-v0-dark-purple text-v0-text-light py-16 md:py-24 lg:py-32">
      <div className="container space-y-12">
        <div className="text-center space-y-4">
          <p className="text-sm font-semibold text-black dark:text-white">Why Choose Our Program</p>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-black dark:text-white">Key Features</h2>
        ...
        {features.map((feature, index) => (
          ...
          >
            <div className="flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full bg-[#246CF4]/20 text-black dark:text-white group-hover:bg-[#246CF4] group-hover:text-white mb-4 transition-colors duration-300">
              <feature.icon className="w-6 h-6 md:w-7 md:h-7" />
            </div>
            <h3 className="text-xl md:text-2xl font-semibold text-black dark:text-white group-hover:text-[#246CF4] mb-2 transition-colors duration-300">{feature.title}</h3>
            <p className="text-sm md:text-base text-black dark:text-white group-hover:text-v0-text-light transition-colors duration-300">{feature.description}</p>
          </div>
        ))}
      </div>
      </div>
